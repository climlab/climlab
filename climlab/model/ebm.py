...
class EBM(EnergyBudget):
    def __init__(self,
                 ...
                 **kwargs):
        initial = grid.zonal_mean_surface(num_lat=num_lat)
        Tarray = (12. - 40. * legendre.P2(np.sin(np.deg2rad(initial.lat.values)))).reshape((initial.lat.size, 1))
        initial['Ts'] = (('lat', 'depth'), Tarray)
        super(EBM, self).__init__(state=initial, timestep=timestep, **kwargs)
        ...
        self.param['ai'] = ai
        # create sub-models
        self.add_subprocess('LW', AplusBT(state=initial, param=self.param))
        self.add_subprocess('insolation',
                            P2Insolation(state=initial, param=self.param))
        #self.add_subprocess('albedo',
        #                    albedo.StepFunctionAlbedo(state=initial,
        #                    param=self.param))

        # diffusivity in units of 1/s
        K = (self.param['D'] / grid.heat_capacity(initial))['Ts'].values
        #K = (self.param['D'] / self.domains['Ts'].grid.heat_capacity(slab))['Ts'].values
        self.add_subprocess('diffusion', MeridionalDiffusion(state=initial,
                                                             K=K,
                                                             param=self.param))
    ...
    def _compute_heating_rates(self):
        ...
        albedo = 0.3  # temp hack to get things going...
