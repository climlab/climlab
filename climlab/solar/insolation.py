...
def instant_insolation(lat, day, lon=0, orb=const.orb_present, S0=const.S0):
    ...
    **Function-call argument** \n
    ...
    # def a constant to make reading code easier
    mins_per_day = 24 * 60
    # local sidereel time in minuites
    LST = np.fmod((np.fmod(day, 1.0) + (lon / 360)) * mins_per_day, mins_per_day)
    # hour angle in rad
    h = np.deg2rad((LST / 4) - 180) 
