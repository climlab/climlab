...
def _Domain(axes=None):
    dom = AttrDict()
    dom['domain_type'] = 'undefined'
    dom['axes'] = _make_axes_dataset(axes)
    dom['grid'] = xgcm.Grid(dom['axes'])
    dom['numdims'] = len(dom['grid'].axes)
    shape = []
    axcount = 0
    axindex = {}
    ...
    add_lon = False
    add_lat = False
    axlist = list(dom['grid'].axes.keys())
    if 'lev' in axlist:
        axlist.remove('lev')
        add_lev = True
    ...
    if add_lev:
        axlist2.append('lev')
    #for axType, ax in dom['axes'].iteritems():
    for axType in axlist2:
        ax = dom['axes'][axType]
        num_points = ax.size
        shape.append(num_points)
        #  can access axes as object attributes
        #setattr(dom, axType, ax)
        #
        axindex[axType] = axcount
        axcount += 1
    dom['axis_index'] = axindex
    dom['shape'] = tuple(shape)
    return dom

def _make_axes_dataset(axes):
...
def Atmosphere(axes=None):
    dom = _Domain(axes)
    dom['domain_type'] = 'atm'
    dom['heat_capacity'] = heat_capacity.atmosphere(compute_delta(dom, 'lev'))
    return dom

def Ocean(axes=None):
    dom = _Domain(axes)
    dom['domain_type'] = 'ocean'
    dom['heat_capacity'] = heat_capacity.ocean(compute_delta(dom, 'depth'))
    return dom
...
def SlabOcean(num_points=1):
    return Ocean(axes=make_slabocean_axis(num_points=num_points))

def SlabAtmosphere(num_points=1):
    return Atmosphere(axes=make_slabatm_axis(num_points=num_points))
...
#     Slab Atmosphere created by :func:`make_slabatm_axis` which has just 1 cell
...
def single_column(num_lev=30, water_depth=1., lev=None, **kwargs):
    ...
    **Function-call argument** \n
    ...
    slab = Ocean(axes=depthax, **kwargs)
...
def zonal_mean_surface(num_lat=90, water_depth=10., lat=None, **kwargs):
    ...
    **Function-call argument** \n
    ...
    depthax = Axis(axis='depth', bounds=[water_depth, 0.])
    axes = xr.merge([depthax,latax])
    slab = Ocean(axes=axes, **kwargs)
    return slab
...
def zonal_mean_column(num_lat=90, num_lev=30, water_depth=10., lat=None,
    ...
    **Function-call argument** \n
    ...
    slab = Ocean(axes={'lat':latax, 'depth':depthax}, **kwargs)
